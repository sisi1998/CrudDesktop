...
public class ShowAllCategoFrontController implements Initializable {
    ...
    public Parent makeCategorieModel(
        ...
        Parent parent = null;
        try {
            parent = FXMLLoader.load(Objects.requireNonNull(getClass().getResource(Constants.FXML_FRONT_MODEL_CATEGORIE)));
    ...
    private void ajouterCategorie(ActionEvent event) {
        ...
        MainWindowFrontController.getInstance().loadInterface(Constants.FXML_FRONT_MANAGE_CATEGORIE);
    }
        ...
        if (action.isPresent()) {
            if (action.get() == ButtonType.OK) {
                if (CategorieService.getInstance().delete(categorie.getId())) {
                    MainWindowFrontController.getInstance().loadInterface(Constants.FXML_FRONT_DISPLAY_ALL_CATEGORIE);
                } else {
